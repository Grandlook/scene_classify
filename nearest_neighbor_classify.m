...
categories = {'Kitchen', 'Store', 'Bedroom', 'LivingRoom', 'Office', ...
 ...
 ind_label=[];  
 for i=1:1500
     ind_y=strcmp(train_labels(i),categories);
     ind_label=[ind_label ; find(ind_y==1)];
 end;

dist=vl_alldist2(test_image_feats',train_image_feats');
% [~,ind]=min(dist');
% predicted_categories=train_labels(ind);
% for i=1:len
%     cursor=test_image_feats(i,:);
%     cur_rep=repmat(cursor',1,len);
%     dist=vl_alldist2(cur_rep,train_image_feats');
%     [~,ind]=min(x);
%     predicted_categories=train_labels(train_labels);
% end;
 ...
 %%取前K个值来统计
 indx=ind(1:12,:);
 label = ind_label(indx);
predicted_categories= mode(label);
