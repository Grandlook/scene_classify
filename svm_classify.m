...
svm_s={};
for i=1:15
        ind_y=strcmp(categories(i),train_labels);
        ind_y=double(ind_y);
        ind_y(ind_y==0)=-1;
       [Wi Bi] = vl_svmtrain(train_image_feats, ind_y', LAMBDA);
        %svmstruct=svmtrain(train_image_feats',ind_y,'kernel_function','rbf');
        %svm_s={svm_s,svmstruct};
        W=[W;Wi'];
        B=[B;Bi];
...
label=[];
 for i=1:1500
        val=W*train_image_feats(:,i)+B;
        [~,ind]=max(val);
        label=[label;ind];
 end;
 predicted_categories=categories(label);
